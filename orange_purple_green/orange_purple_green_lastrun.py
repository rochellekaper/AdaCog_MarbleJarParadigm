...
# Run 'Before Experiment' code from exp_name
exp_name = 'orange_purple_green'
...
expName = 'orange_purple_green'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\kaperr.WIN\\Desktop\\Marble_Jar_Paradigm\\orange_purple_green\\orange_purple_green_lastrun.py',
...
# --- Initialize components for Routine "exp_name3" ---
...
# --- Prepare to start Routine "exp_name3" ---
continueRoutine = True
routineForceEnded = False
# update component parameters for each repeat
# keep track of which components have finished
exp_name3Components = []
for thisComponent in exp_name3Components:
...
# --- Ending Routine "exp_name3" ---
for thisComponent in exp_name3Components:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# the Routine "exp_name3" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
...
practice_trials = data.TrialHandler(nReps=1.0, method='fullRandom', 
    ...
    trialList=data.importConditions('practice_orange_purple_green.xlsx'),
...
for thisPractice_trial in practice_trials:
    ...
    while continueRoutine:
        ...
        waitOnFlip = False
        if key_resp_trials_2.status == NOT_STARTED and tThisFlip >= 3.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(key_resp_trials_2.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(key_resp_trials_2.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if key_resp_trials_2.status == STARTED and not waitOnFlip:
...
# set up handler to look after randomisation of conditions etc
which_task = data.TrialHandler(nReps=1.0, method='fullRandom', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Condition_File_orange_purple_green.xlsx'),
...
for thisWhich_task in which_task:
    ...
    orange1 = list(Jar1_Colors).count('darkorange') * color_pts_dict['darkorange']
    green1 = list(Jar1_Colors).count('green') * color_pts_dict['green']
    purple1 = list(Jar1_Colors).count('purple') * color_pts_dict['purple']
    
    jar1_totalpoints = orange1 + green1 + purple1
    thisExp.addData('marbleJar1_SumOfPoints', jar1_totalpoints)
    
    orange2 = list(Jar2_Colors).count('darkorange') * color_pts_dict['darkorange']
    green2 = list(Jar2_Colors).count('green') * color_pts_dict['green']
    purple2 = list(Jar2_Colors).count('purple') * color_pts_dict['purple']
    
    jar2_totalpoints = orange2 + green2 + purple2
    thisExp.addData('marbleJar2_SumOfPoints', jar2_totalpoints)
    
    orange3 = list(Jar3_Colors).count('darkorange') * color_pts_dict['darkorange']
    green3 = list(Jar3_Colors).count('green') * color_pts_dict['green']
    purple3 = list(Jar3_Colors).count('purple') * color_pts_dict['purple']
    ...
    which_task.addData('key_resp_break.keys',key_resp_break.keys)
    if key_resp_break.keys != None:  # we had a response
        which_task.addData('key_resp_break.rt', key_resp_break.rt)
